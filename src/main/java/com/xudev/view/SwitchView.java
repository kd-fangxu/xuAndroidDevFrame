...
public class SwitchView extends LinearLayout {
	private ImageView maskImage; // �?关遮盖图�?
	private boolean open; // �?关当前状�?
	private boolean isAninFinish = true; // 动画是否结束
	private float x; // 记录ACTION_DOWN时�?�的横坐�?
	private boolean isChangedByTouch = false; // 是否在一次事件中已经切换过状�?
	private OnSwitchChangeListener switchChangeListener; // 监控�?关状�?
	...
	private void changeOpenStatusWithAnim(boolean open) {
		if (open) {
			// 左到�?
			Animation leftToRight = new TranslateAnimation(Animation.RELATIVE_TO_SELF, 0, Animation.ABSOLUTE,
					getWidth() - maskImage.getWidth(), Animation.RELATIVE_TO_SELF, 0, Animation.RELATIVE_TO_SELF, 0);
			...
			// 右到�?
			Animation rightToLeft = new TranslateAnimation(Animation.RELATIVE_TO_SELF, 0, Animation.ABSOLUTE,
					maskImage.getWidth() - getWidth(), Animation.RELATIVE_TO_SELF, 0, Animation.RELATIVE_TO_SELF, 0);
