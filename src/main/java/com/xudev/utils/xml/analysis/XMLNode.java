package com.xudev.utils.xml.analysis;
...
public class XMLNode {
	...
	public XMLNode(String nodeName, String nodeValue, XMLNodeAtt attribute, List<XMLNode> childList) {
		super();
		this.nodeName = nodeName;
		this.nodeValue = nodeValue;
		this.attribute = attribute;
	...
	public boolean isNodeNameEquals(XMLNode node) {
		if (this.nodeName != null && node.nodeName != null && this.nodeName.equals(node.nodeName)) {
			return true;
		}
		return false;
	}
	 ...
	 * @param tagName
	 *            指定的下级节点名称
	 * @param isFirstLevelChild
	 *            是否只查找第一级子节点，true：是，false：否
	 * @return
	 * @throws Exception
	 */
	 ...
	 *            节点名称
	 * @param nodeAtt
	 *            节点属性，属性如果为空则默认只按照节点名称进行查找匹配
	 * @param isFirstLevelChild
	 *            是否只查找第一级子节点，true：是，false：否
	 * @return
	 * @throws Exception
	 */
	public List<XMLNode> getNodes(String tagName, XMLNodeAtt nodeAtt, boolean isFirstLevelChild) throws Exception {
		if (isEmpty(tagName)) {
			throw new Exception("标签名称不能为空");
		}
	...
	public void setChildNodeValue(String childNodeName, String childNodeValue) {
		for (int i = 0; i < childList.size(); i++) {
			if (childNodeName.equals(childList.get(i).getNodeName()) && !childList.get(i).isHasChild()) {
	...
	public boolean equals(Object o) {
		...
		if (!tnode.getNodeName().equals(this.nodeName) || !tnode.getNodeValue().equals(this.nodeValue)) {
			return false;
		}
